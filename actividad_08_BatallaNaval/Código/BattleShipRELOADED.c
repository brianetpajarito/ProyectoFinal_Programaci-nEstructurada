...
///Mecanismo de ataque///
    ...
    FILE *archivo;
    archivo = fopen("partida.txt", "w");//abrimos el archvio para guardar partida
    ...
    fprintf(archivo,"Puntuacion final: %s:%d | %s:%d\n", players[0],victoria1,players[1],victoria2);
    fprintf(archivo,"\nTABLERO DE %s\n",players[0]);//IMPRIMIR UBICACIONES
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            if(i == 0 || j == 0){
                fprintf(archivo,"%d   ", jugador1[i][j]);
            }
            else{
    ...
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            if(i == 0 || j == 0){
                fprintf(archivo,"%d   ", jugador2[i][j]);
            }
            else{
                fprintf(archivo,"%c   ", jugador2[i][j]);
            }
        }
        fprintf(archivo,"\n\n");
    }
    ...
    printf("Quieres ver la partida guardada?\n");
    printf("Si[7]\n");
    printf("No[cualquier numero]\n");
    scanf("%d",&revisar);
    if(revisar==7){
        printf("\nAqui esta la ultima partida\n");

        FILE *lectura = fopen("partida.txt", "r");
    if (lectura == NULL) {
        perror("Error al abrir archivo para lectura");
        return 1;
    }

    char linea[256];//buffer para cada línea
    while (fgets(linea, sizeof(linea), lectura)) {
        printf("%s", linea);//imprimir línea en pantalla
    }

    fclose(lectura);
    }
    else{
        printf("\nHasta la proxima, vuelve protno a jugar. :)\n");
    }
